...
from vyos.utils.auth import DEFAULT_PASSWORD, EPasswdStrength, evaluate_strength
...
MSG_ERR_NOT_LIVE: str = (
    'The system is already installed. Please use "add system image" instead.'
)
MSG_ERR_LIVE: str = (
    'The system is in live-boot mode. Please use "install image" instead.'
)
MSG_ERR_NOT_ENOUGH_SPACE: str = (
    "Image upgrade requires at least 2GB of free drive space."
)
MSG_ERR_NO_DISK: str = (
    "No suitable disk was found. There must be at least one disk of 2GB or greater size."
)
MSG_ERR_IMPROPER_IMAGE: str = (
    "Missing sha256sum.txt.\nEither this image is corrupted, or of era 1.2.x (md5sum) and would downgrade image tools;\ndisallowed in either case."
)
MSG_ERR_INCOMPATIBLE_IMAGE: str = (
    "Image compatibility check failed, aborting installation."
)
MSG_ERR_ARCHITECTURE_MISMATCH: str = (
    'The current architecture is "{0}", the new image is for "{1}". Upgrading to a different image architecture will break your system.'
)
MSG_ERR_FLAVOR_MISMATCH: str = (
    'The current image flavor is "{0}", the new image is "{1}". Upgrading to a non-matching flavor can have unpredictable consequences.'
)
MSG_ERR_MISSING_ARCHITECTURE: str = (
    "The new image version data does not specify architecture, cannot check compatibility (is it a legacy release image?)"
)
MSG_ERR_MISSING_FLAVOR: str = (
    "The new image version data does not specify flavor, cannot check compatibility (is it a legacy release image?)"
)
MSG_ERR_CORRUPT_CURRENT_IMAGE: str = (
    "Version data in the current image is malformed: missing flavor and/or architecture fields. Upgrade compatibility cannot be checked."
)
MSG_ERR_UNSUPPORTED_SIGNATURE_TYPE: str = (
    "Unsupported signature type, signature cannot be verified."
)
MSG_INFO_INSTALL_WELCOME: str = (
...
MSG_INFO_INSTALL_RAID_FOUND_DISKS: str = (
    "Would you like to configure RAID-1 mirroring on them?"
)
MSG_INFO_INSTALL_RAID_CHOOSE_DISKS: str = (
    "Would you like to choose two disks for RAID-1 mirroring?"
)
MSG_INFO_INSTALL_DISK_CONFIRM: str = (
    "Installation will delete all data on the drive. Continue?"
)
MSG_INFO_INSTALL_RAID_CONFIRM: str = (
    "Installation will delete all data on both drives. Continue?"
)
MSG_INFO_INSTALL_PARTITONING: str = "Creating partition table..."
MSG_INPUT_CONFIG_FOUND: str = (
...
MSG_INPUT_ROOT_SIZE_SET: str = (
    "Please specify the size (in GB) of the root partition (min is 1.5 GB)?"
)
MSG_INPUT_CONSOLE_TYPE: str = (
...
MSG_INPUT_CHOOSE_COPY_DATA: str = (
    "From which image would you like to save config information?"
)
MSG_INPUT_COPY_ENC_DATA: str = (
    "Would you like to copy the encrypted config to the new image?"
)
MSG_INPUT_CHOOSE_COPY_ENC_DATA: str = (
    "From which image would you like to copy the encrypted config?"
)
MSG_WARN_ISO_SIGN_INVALID: str = (
    "Signature is not valid. Do you want to continue with installation?"
)
MSG_WARN_ISO_SIGN_UNAVAL: str = (
...
MSG_WARN_IMAGE_NAME_WRONG: str = (
    "The suggested name is unsupported!\n"
    "It must be between 1 and 64 characters long and contains only the next characters: .+-_ a-z A-Z 0-9"
)

MSG_WARN_CHANGE_PASSWORD: str = (
    "Default password used. Consider changing " "it on next login."
)
...
def create_partitions(target_disk: str, target_size: int, prompt: bool = True) -> None:
    ...
    disk_details: disk.DiskDetails = disk.parttable_create(target_disk, rootfs_size)
...
def search_previous_installation(disks: list[str]) -> None:
    ...
    if len(image_data) > 0:
        if len(image_data) == 1:
            ...
            if ask_yes_no(MSG_INPUT_COPY_DATA, default=True):
                image_name, image_drive = select_entry(
                    image_data,
                    "Available versions:",
...
def copy_previous_installation_data(target_dir: str) -> None:
    if Path("/mnt/config").exists():
        copytree("/mnt/config", f"{target_dir}{DIR_CONFIG}", dirs_exist_ok=True)
    if Path("/mnt/ssh").exists():
        copytree("/mnt/ssh", f"{target_dir}/etc/ssh", dirs_exist_ok=True)
...
def ask_single_disk(disks_available: dict[str, int]) -> str:
    ...
    disk_selected: str = ask_input(
        MSG_INFO_INSTALL_DISK_SELECT,
...
def check_raid_install(disks_available: dict[str, int]) -> Union[str, None]:
    ...
    disks_selected: dict[str, int] = {
        disk0: disks_available[disk0],
    ...
    for disk_selected in list(disks_selected):
        ...
        disk_details = create_partitions(disk_selected, target_size, prompt=False)
        disk.filesystem_create(disk_details.partition["efi"], "efi")
...
def configure_authentication(config_file: str, password: str) -> None:
    ...
    config.set(
        ["system", "login", "user", "vyos", "authentication", "encrypted-password"],
...
def validate_signature(file_path: str, sign_type: str) -> None:
    ...
    if sign_type == "minisig":
        ...
        for pubkey in pub_key_list:
            if (
                run(f"minisign -V -q -p {pubkey} -m {file_path} -x {file_path}.minisig")
                == 0
            ):
    ...
    try:
        ...
        if urlparse(image_path).scheme:
            ...
            ISO_DOWNLOAD_PATH = os.path.join(
                os.path.expanduser("~"), "{0}.iso".format(uuid4())
            )
            download_file(
                ISO_DOWNLOAD_PATH, image_path, vrf, progressbar=True, check_space=True
            )
            ...
            if sign_file[0]:
                ...
                if not no_prompt and not ask_yes_no(
                    MSG_WARN_ISO_SIGN_UNAVAL, default=False
                ):
                    cleanup()
                    exit(MSG_INFO_INSTALL_EXIT)
...
def cleanup(mounts: list[str] = [], remove_items: list[str] = []) -> None:
    ...
    if Path(ISO_DOWNLOAD_PATH).exists() and ISO_DOWNLOAD_PATH not in remove_items:
...
def cleanup_raid(details: raid.RaidDetails) -> None:
    ...
    cleanup([details.name, *efiparts], ["/mnt/installation"])
...
def validate_compatibility(iso_path: str, force: bool = False) -> None:
    ...
    current_architecture = current_data.get("architecture") or cmd(
    ...
    if current_architecture != new_architecture:
        ...
        if not new_architecture:
            ...
            print(
                MSG_ERR_ARCHITECTURE_MISMATCH.format(
...
def install_image() -> None:
    ...
    DIR_DST_ROOT = "/lib/live/mount/persistence"
    ...
    while True:
        image_name: str = ask_input(MSG_INPUT_IMAGE_NAME, running_image_name)
    ...
    console_type: str = ask_input(
        MSG_INPUT_CONSOLE_TYPE, default=console_hint(), valid_responses=["K", "S"]
    )
    ...
    try:
        ...
        if False:
            ...
            for disk_target in l:
                disk.partition_mount(
                    disk_target.partition["efi"], f"{DIR_DST_ROOT}/boot/efi"
                )
                grub.install(
                    disk_target.name,
                    f"{DIR_DST_ROOT}/boot/",
            ...
            grub.install(
                install_target.name, f"{DIR_DST_ROOT}/boot/", f"{DIR_DST_ROOT}/boot/efi"
            )
...
def add_image(
    ...
    try:
        ...
        disk.partition_mount(
            f"{DIR_ISO_MOUNT}/live/filesystem.squashfs", DIR_ROOTFS_SRC, "squashfs"
        )
        ...
        if no_prompt or migrate_config():
            if Path("/dev/mapper/vyos_config").exists():
                ...
                copytree(
                    ...
                    target_config_dir,
                    symlinks=True,
                    copy_function=copy_preserve_owner,
                    dirs_exist_ok=True,
                )
        ...
        copy(
            f"{DIR_ISO_MOUNT}/live/filesystem.squashfs",
        ...
        if Path(f"{target_config_dir}/config.boot").exists():
            cmdline_options = get_cli_kernel_options(f"{target_config_dir}/config.boot")
            grub_util.update_kernel_cmdline_options(
                " ".join(cmdline_options), root_dir=root_dir, version=image_name
            )
        ...
        if e.errno == ENOSPC:
            cleanup(
                mounts=[str(iso_path)], remove_items=[f"{root_dir}/boot/{image_name}"]
            )
        else:
            # unmount an ISO and cleanup
            cleanup([str(iso_path)])
...
def parse_arguments() -> Namespace:
    ...
    """
    parser: ArgumentParser = ArgumentParser(description="Install new system images")
    parser.add_argument(
        "--action",
        choices=["install", "add"],
        required=True,
    ...
    parser.add_argument("--vrf", help="vrf name for image download")
    parser.add_argument(
        "--no-prompt", action="store_true", help="perform action non-interactively"
    )
    parser.add_argument("--username", default="", help="username for image download")
    parser.add_argument("--password", default="", help="password for image download")
    parser.add_argument(
        "--image-path",
        help="a path (HTTP or local file) to an image that needs to be installed",
    )
    parser.add_argument(
        "--force", action="store_true", help="Ignore flavor compatibility requirements."
    )
